...
SELECT
    ...
    ISNULL(pk_col.column_name,'无主键') AS 主键字段,
    COUNT(c.column_id) AS 字段总数,
    SUM(p.rows) AS 数据量行数,
    t.modify_date AS 最后修改时间,
    COUNT(i.index_id) AS 索引数量,
    '否' AS 是否分区表,
    'DBA' AS 负责人,
    '业务表' AS 备注
FROM sys.tables t
...
LEFT JOIN (
    ...
    WHERE ic.index_id=1
) pk_col ON t.object_id=pk_col.object_id
LEFT JOIN sys.columns c ON t.object_id=c.object_id
WHERE t.is_ms_shipped=0
GROUP BY t.name,SCHEMA_NAME(t.schema_id),ep.value,pk_col.column_name,t.modify_date,p.rows
ORDER BY t.name
...
SELECT 
    ...
    d.state_desc AS 状态,
    d.recovery_model_desc AS 恢复模式,
    d.compatibility_level AS 兼容级别,
    d.collation_name AS 排序规则,
    d.create_date AS 创建时间,
    d.owner_sid AS 所有者SID,
    ...
    '' AS 备注
FROM sys.databases d
WHERE d.database_id > 4                    -- 排除系统库（可根据需要去掉）
ORDER BY d.name;
